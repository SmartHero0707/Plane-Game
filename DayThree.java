...
class ShootGame extends JPanel{ 
	...
	int shootIndex=0,eplane=0;//子弹发射计数 敌军计数
	int count=10;
	private Bullet[] bullets={};
	private EmPlane[] flyings={};
	 ShootGame(){
		 	setLayout(null);
			// n.setBounds(40, 20,120, 20);
			 ...
			 timer.schedule(new TimerTask(){
				public void run() {
						if(hero.y>550&&Static.state==0) {
						      hero.y-=3;
						      hero.step();
					 }
					if(Static.state==Static.RUNNING){
					Heromove();
					shootAction();
					EplaneAction();
					bandAction();
	...
	/*英雄随着键盘上下左右键移动*/
	 ...
	 public EmPlane nextOne(){
			...
			if(count==0){
				return new EmPlane(new File("./image/airplane.png"),new File("./image/airplane.png"),200,5);
			}
			else if(count==2){
				return new EmPlane(new File("./image/bee.png"),new File("./image/airplane.png"),10000,8);
			}
			else if(count==3) {
				return new EmPlane(new File("./image/airplane.png"),new File("./image/airplane.png"),800,2);
			}else{
				return new EmPlane(new File("./image/airplane.png"),new File("./image/airplane.png"),200,1);
			}	
		}
	 /** 敌机入场*/
     ...
     public void emPlaneBang(Bullet bullet) {
    	 int index=-1;
    	 int life =bullet.getLife();
	 ...
	 private void painEPlane(Graphics g) {
		 for(EmPlane e:flyings) {
			 if(Static.state==1&&e.getState()==Static.LIFE)e.move();
			 //e.step();
	 ...
	 private void paintScore(Graphics g) {
		 ...
		 if(Static.state==1)
		 g.drawString("Score:"+Static.Score,10,70);
	 }
...
class EmPlane extends FlyingObject{
	...
	EmPlane(File f1,File f2,int life,int speed){
		...
		this.speed=speed;
		try {
			images=new BufferedImage[]{ ImageIO.read(f1), ImageIO.read(f2)};
		}
		catch(IOException e) {	
		}
		}
...
class Bullet extends FlyingObject{
	...
	@Override
	protected boolean hit(FlyingObject b) {
		// TODO Auto-generated method stub
		return false;
	}
	@Override
...
abstract class FlyingObject{
	...
	//protected abstract boolean hit(FlyingObject b);
	public void setLife(int life) {
		// TODO Auto-generated method stub
...
class ImplAWTEventListener implements AWTEventListener { 
    ...
    public void eventDispatched(AWTEvent event){  
    	if (event.getClass() == KeyEvent.class) {  
            ...
            if ((char)keyEvent.getKeyChar()==KeyEvent.VK_Z) {
